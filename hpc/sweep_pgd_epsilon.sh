...
# NOTE: the -o/-e log directory must already exist at submit time (Slurm opens
# these before the script body runs). /ptmp/$USER/atoms_test_alt is created by
# the test-set sync (it holds frames/), so it exists for the default paths; if
# you point FRAMES_DIR elsewhere, create the log dir first or the job won't log.
#SBATCH -J pgd_eps_sweep
#SBATCH -o /ptmp/%u/atoms_test_alt/pgd_eps_sweep_%j.out
#SBATCH -e /ptmp/%u/atoms_test_alt/pgd_eps_sweep_%j.err
#SBATCH --ntasks=1
...
# On Viper the test frames live under /ptmp (they are NOT synced into the repo);
# see docs/cluster_explanations.md — alt split → /ptmp/$USER/atoms_test_alt/frames.
FRAMES_DIR="${FRAMES_DIR:-/ptmp/$USER/atoms_test_alt/frames}"
...
mkdir -p /ptmp/$USER/atoms_test_alt "$(dirname "$OUT")"
