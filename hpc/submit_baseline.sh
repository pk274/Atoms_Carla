...
# submit_baseline.sh
# ------------------
# Discover all run_*.npz files, submit a SLURM array job (one task per file),
# then chain a gather job that runs after all tasks succeed.
#
# Usage (from $CODE_DIR on the HPC):
#   bash hpc/submit_baseline.sh <FRAMES_DIR> <PARTIALS_DIR> <MODEL_DIR> [CODE_DIR]
#
# Arguments:
#   FRAMES_DIR   directory containing run_*.npz files
#                e.g. /ptmp/$USER/atoms_baseline/frames
#   PARTIALS_DIR output directory for partial results and final baseline.npz
#                e.g. /ptmp/$USER/atoms_baseline/partials
...
#       /ptmp/$USER/atoms_baseline/frames \
#       /ptmp/$USER/atoms_baseline/partials \
...
# Write the file list so array tasks can index it
...
echo ""
echo "After gather completes, copy result locally with:"
echo "  rsync -avz \$USER@viper.mpcdf.mpg.de:$PARTIALS_DIR/baseline.npz data/TFV6/baseline_data/"
