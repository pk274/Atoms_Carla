#!/bin/bash -l
# array_task_wor.sh
# Per-task script for the WoR baseline SLURM array job.
...
#   MODEL_DIR    path to the WoR pretrained weights directory
#   CODE_DIR     project root

#SBATCH -J atoms_wor_baseline
#SBATCH -o /ptmp/%u/atoms_wor_baseline/logs/chunk_%A_%a.out
#SBATCH -e /ptmp/%u/atoms_wor_baseline/logs/chunk_%A_%a.err
#SBATCH --ntasks=1
...
# pcla_agents/transfuserv6 is not needed for WoR but harmless to include.
export PYTHONPATH="$CODE_DIR/hpc/stubs:$CODE_DIR:$PYTHONPATH"

mkdir -p "$PARTIALS_DIR" /ptmp/$USER/atoms_wor_baseline/logs

# Resolve this task's run file (LIST_FILE is 0-indexed via sed offset +1)
RUN_FILE=$(sed -n "$((SLURM_ARRAY_TASK_ID + 1))p" "$LIST_FILE")
OUTPUT="$PARTIALS_DIR/partial_${SLURM_ARRAY_TASK_ID}.npz"

echo "=== WoR Baseline Task $SLURM_ARRAY_TASK_ID ==="
